---
version: alpha
name: gooru-portfolio-design
description: "A near-black product-focused marketing canvas built around #010102 (the deepest dark surface of any tool in this collection), light gray text (#f7f8f8), and the signature Linear amber-gold (#e2b13c) used as the single chromatic accent. The system reads as software-craft documentation: dense, technical, and quietly luxurious. Display type is set in the Linear custom sans (SF Pro Display fallback) at 500–700 with measured negative tracking. Cards live as charcoal panels (#0f1011) with hairline borders. The accent amber appears on the brand mark, focus rings, and a few intentional CTAs — never decoratively. Page rhythm leans on product UI screenshots framed in dark panels rather than atmospheric color."

colors:
  primary: "#e2b13c"
  on-primary: "#ffffff"
  primary-hover: "#f0c85a"
  primary-focus: "#c99a28"
  ink: "#f7f8f8"
  ink-muted: "#d0d6e0"
  ink-subtle: "#8a8f98"
  ink-tertiary: "#62666d"
...
---
---
# Adapted for gooru portfolio from VoltAgent/awesome-design-md → linear.app/DESIGN.md
# Accent remapped from Linear lavender to gooru amber; display face = Sora.
...
The single chromatic accent is **Linear amber-gold** `{colors.primary}` (#e2b13c) — used on the brand mark, focus rings, and the primary CTA button. A lighter hover state (`{colors.primary-hover}` #f0c85a) and a focus-tinted variant (`{colors.primary-focus}` #c99a28) extend the same hue. Linear avoids saturated greens, oranges, reds, etc. on the marketing canvas — the only semantic color is `{colors.semantic-success}` (#27a644) for status pills and the rare success indicator.

Display type runs Linear's custom sans (with `SF Pro Display` fallback) at weight 500–700 with negative letter-spacing scaling from -3.0px at 80px down to 0 at body. The body family is Linear's text cut, and a IBM Plex Mono is reserved for code snippets in product screenshots.

The page rhythm is **dense product screenshots** — Linear's marketing leads with high-fidelity captures of the product UI (issue list, project view, dashboard) framed in `{colors.surface-1}` panels with `{rounded.xl}` 16px corners. The chrome is intentionally minimal so the app screenshots can do the heavy lifting.

**Key Characteristics:**
- **Dark-canvas marketing system** — `{colors.canvas}` (#010102) is the deepest dark in this collection.
- **Lavender-blue brand accent** (`{colors.primary}` #e2b13c) — used scarcely on brand mark, focus, and the primary CTA.
...
- **Lavender Hover** ({colors.primary-hover}): Lighter amber (#f0c85a) — hovered state of the primary CTA.
- **Lavender Focus** ({colors.primary-focus}): Focus-ring tint (#c99a28) — focused inputs, focused buttons.
- **Brand Secure** ({colors.brand-secure}): Muted amber-gray (#a89050) — used in "Linear Security" surfaces.
...
- **Sora** — Linear's custom display sans; fallback `SF Pro Display, -apple-system, system-ui, Segoe UI, Roboto`. Carries display-xl through subhead.
- **IBM Plex Sans** — Linear's custom text sans (a slightly different cut tuned for body sizes); same fallback stack. Carries body sizes, button labels, captions.
- **IBM Plex Mono** — Linear's custom mono; fallback `ui-monospace, SF Mono, Menlo`. Used for code snippets in product screenshots and for status / ID tokens.
...
| `{typography.mono}` | 13px | 400 | 1.50 | 0 | IBM Plex Mono for code in product screenshots |
...
- **Mono only in code contexts.** IBM Plex Mono lives inside product screenshots — not on marketing chrome.
...
- Hover state lives in `button-primary-hover` (background shifts to `{colors.primary-hover}` lighter amber).
...
- Use `{colors.primary}` amber ONLY for: brand mark, primary CTA, focus ring, link emphasis.
...
- Don't use amber as a section background or card fill.
...
6. Treat amber as scarce: brand mark, primary CTA, focus, link emphasis.
